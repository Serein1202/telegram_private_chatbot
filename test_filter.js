...
const src = fs
  .readFileSync(__dirname + "/worker.js", "utf8")
...
vm.runInContext(
  src +
    "\nmodule.exports = { analyzeContentSignals, filterMessage, CONFIG, isTurnstileEnabled };",
  sandbox,
  { filename: "worker.js" },
);

const { analyzeContentSignals, filterMessage, CONFIG, isTurnstileEnabled } =
...
const cases = [
  ...
  [
    "正常带链接",
    { text: "你好，这个问题我之前在 https://example.com/a 看到过说明" },
  ],
  [
    "刷单广告",
    { text: "宝妈兼职刷单返利，动动手指日入300，加微信 abc12345 详聊" },
  ],
...
(async () => {
  console.log(
    "阈值: block>=" +
      CONFIG.FILTER_BLOCK_SCORE +
      ", gray>=" +
      CONFIG.FILTER_GRAY_SCORE,
    "\n",
  );
