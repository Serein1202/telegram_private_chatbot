...
const LOCAL_QUESTIONS = [
  ...
  {
    question: "在天上飞的交通工具是什么？",
    correct_answer: "飞机",
    incorrect_answers: ["汽车", "轮船", "自行车"],
  },
  {
    question: "星期一的后面是星期几？",
    correct_answer: "星期二",
    incorrect_answers: ["星期日", "星期五", "星期三"],
  },
  {
    question: "鱼通常生活在哪里？",
    correct_answer: "水里",
    incorrect_answers: ["树上", "土里", "火里"],
  },
  {
    question: "我们用什么器官来听声音？",
    correct_answer: "耳朵",
    incorrect_answers: ["眼睛", "鼻子", "嘴巴"],
  },
  {
    question: "晴朗的天空通常是什么颜色的？",
    correct_answer: "蓝色",
    incorrect_answers: ["绿色", "红色", "紫色"],
  },
  {
    question: "太阳从哪个方向升起？",
    correct_answer: "东方",
...
function isTestMessageInvalid(description) {
  const desc = normalizeTgDescription(description);
  return (
    desc.includes("message text is empty") ||
...
async function getOrCreateAnomalyTopic(env) {
  ...
  const p = (async () => {
    ...
    if (!res.ok)
      throw new Error(
        `创建【${CONFIG.ANOMALY_TOPIC_TITLE}】话题失败: ${res.description}`,
      );
    ...
    await env.TOPIC_MAP.put(
      ...
      JSON.stringify({
        thread_id: threadId,
        title: CONFIG.ANOMALY_TOPIC_TITLE,
      }),
    );
 ...
 */
async function probeForumThread(
  env,
  expectedThreadId,
  { userId, reason, doubleCheckOnMissingThreadId = true } = {},
) {
  ...
  if (second.status === "missing_thread_id") {
    Logger.warn("thread_probe_missing_thread_id", {
      userId,
      expectedThreadId,
      reason,
    });
...
async function resetUserVerificationAndRequireReverify(
  env,
  { userId, userKey, oldThreadId, pendingMsgId, reason },
) {
  // 清理旧映射与验证状态：用户需要重新做人机验证
  await env.TOPIC_MAP.delete(`verified:${userId}`);
  await env.TOPIC_MAP.put(`needs_verify:${userId}`, "1", {
    expirationTtl: CONFIG.NEEDS_REVERIFY_TTL_SECONDS,
  });
...
async function isAdminUser(env, userId) {
  ...
  try {
    ...
    const isAdmin =
      res.ok && (status === "creator" || status === "administrator");
    await env.TOPIC_MAP.put(kvKey, isAdmin ? "1" : "0", {
      expirationTtl: CONFIG.ADMIN_CACHE_TTL_SECONDS,
    });
    adminStatusCache.set(cacheKey, { ts: now, isAdmin });
    return isAdmin;
  } catch (e) {
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (!env.SUPERGROUP_ID)
      return new Response("Error: SUPERGROUP_ID not set.");
...
async function forwardToTopic(msg, userId, key, env, ctx) {
  ...
  if (needsVerify) {
    await sendVerificationChallenge(
      userId,
      env,
      msg.message_id || null,
      extractMsgText(msg),
  ...
  if (rec && rec.thread_id) {
    ...
    if (!withinTTL) {
      ...
      if (kvHealthOk === "1") {
        ...
        const probe = await probeForumThread(env, rec.thread_id, {
          userId,
          reason: "health_check",
        });

        if (
          probe.status === "redirected" ||
          ...
          threadHealthCache.set(cacheKey, { ts: now, ok: true });
          await env.TOPIC_MAP.put(kvHealthKey, "1", {
  ...
  if (filterResult.action === "block") {
    ...
    if (strikes >= CONFIG.FILTER_STRIKE_LIMIT) {
      ...
      await env.TOPIC_MAP.put(strikeKey, String(strikes), {
        expirationTtl: CONFIG.FILTER_STRIKE_TTL_SECONDS,
      });
    }
    ...
    await tgCall(env, "sendMessage", {
      chat_id: userId,
      text:
        strikes >= CONFIG.FILTER_STRIKE_LIMIT
  ...
  if (filterResult.action === "flag") {
    ...
    try {
      ...
      try {
        await env.TOPIC_MAP.delete(ANOMALY_TOPIC_KEY);
      } catch (_) {}
    }
    return "flag";
  }
  ...
  if (res.ok && (resThreadId === undefined || resThreadId === null)) {
    const probe = await probeForumThread(env, rec.thread_id, {
      userId,
      reason: "forward_result_missing_thread_id",
    });
    if (probe.status !== "ok") {
  ...
  if (!res.ok) {
    ...
    if (desc.includes("chat not found"))
      throw new Error(`群组ID错误: ${env.SUPERGROUP_ID}`);
    if (desc.includes("not enough rights"))
      throw new Error("机器人权限不足 (需 Manage Topics)");
...
async function handleAdminReply(msg, env, ctx) {
  ...
  if (text === "/info") {
    ...
    const safeTitle = (userRec?.title || "未知")
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;");
...
async function sendVerificationChallenge(
      ...
      if (pendingMsgId) {
        ...
        if (!pendingIds.includes(pendingMsgId)) {
          ...
          if (pendingIds.length > CONFIG.PENDING_MAX_MESSAGES) {
            pendingIds = pendingIds.slice(
              pendingIds.length - CONFIG.PENDING_MAX_MESSAGES,
            );
          }
          ...
          await env.TOPIC_MAP.put(chalKey, JSON.stringify(state), {
            expirationTtl: CONFIG.VERIFY_EXPIRE_SECONDS,
          });
        }
      }
      Logger.debug("verification_duplicate_skipped", {
        userId,
        verifyId: existingChallenge,
  ...
  const state = {
    ...
    pending_texts:
      pendingMsgId && pendingText
        ? { [String(pendingMsgId)]: pendingText }
        : {},
    userId: userId, // 添加用户ID验证
  };

  await env.TOPIC_MAP.put(`chal:${verifyId}`, JSON.stringify(state), {
    expirationTtl: CONFIG.VERIFY_EXPIRE_SECONDS,
  });

  // 【修复 #1】标记用户正在验证中
  await env.TOPIC_MAP.put(`user_challenge:${userId}`, verifyId, {
    expirationTtl: CONFIG.VERIFY_EXPIRE_SECONDS,
  });
...
async function handleCallbackQuery(query, env, ctx) {
  try {
    ...
    if (
      isNaN(selectedIndex) ||
      selectedIndex < 0 ||
      selectedIndex >= state.options.length
    ) {
      await tgCall(env, "answerCallbackQuery", {
        callback_query_id: query.id,
        text: "❌ 无效选项",
    ...
    if (selectedIndex === state.answerIndex) {
      ...
      const hasPending =
        (Array.isArray(state.pending_ids) && state.pending_ids.length > 0) ||
        !!state.pending;
      if (hasPending) {
        try {
          let pendingIds = [];
          ...
          for (const pendingId of pendingIds) {
            ...
            const fakeMsg = {
              ...
              text:
                (state.pending_texts &&
                  state.pending_texts[String(pendingId)]) ||
                "",
            };

            const fwdResult = await forwardToTopic(
              fakeMsg,
              userId,
              `user:${userId}`,
              env,
...
async function sendTurnstileChallenge(userId, env, pendingMsgId, pendingText) {
  ...
  if (existingToken) {
    ...
    if (state && state.userId === userId) {
      if (pendingMsgId) {
        let pendingIds = Array.isArray(state.pending_ids)
          ? state.pending_ids.slice()
          : [];
        if (!pendingIds.includes(pendingMsgId)) {
          pendingIds.push(pendingMsgId);
          if (pendingIds.length > CONFIG.PENDING_MAX_MESSAGES) {
            pendingIds =
              pendingIds.length - CONFIG.PENDING_MAX_MESSAGES > 0
                ? pendingIds.slice(
                    pendingIds.length - CONFIG.PENDING_MAX_MESSAGES,
                  )
                : pendingIds;
          ...
          await env.TOPIC_MAP.put(stateKey, JSON.stringify(state), {
            expirationTtl: CONFIG.TURNSTILE_LINK_TTL_SECONDS,
          });
        }
      }
      Logger.debug("turnstile_duplicate_skipped", {
        userId,
        hasPending: !!pendingMsgId,
      });
      return;
  ...
  await env.TOPIC_MAP.put(`ts:${linkToken}`, JSON.stringify(state), {
    expirationTtl: CONFIG.TURNSTILE_LINK_TTL_SECONDS,
  });
  await env.TOPIC_MAP.put(`user_challenge:${userId}`, linkToken, {
    expirationTtl: CONFIG.TURNSTILE_LINK_TTL_SECONDS,
  });
  ...
  if (!origin) {
    Logger.error(
      "turnstile_origin_missing",
      new Error("无法确定 Worker 公网地址，请配置 PUBLIC_BASE_URL"),
    );
    return;
  }
...
async function handleTurnstilePage(request, env) {
  if (!isTurnstileEnabled(env)) {
    return new Response(
      "Turnstile 未配置：请设置 TURNSTILE_SITEKEY 与 TURNSTILE_SECRET 环境变量。",
      { status: 404 },
    );
  }
  const token = (new URL(request.url).searchParams.get("t") || "").replace(
    /[^a-zA-Z0-9]/g,
    "",
  );
...
async function handleTurnstileComplete(request, env, ctx) {
  ...
  if (!isTurnstileEnabled(env))
    return jsonResp({ ok: false, message: "Turnstile 未配置" }, 404);
  ...
  if (!linkToken || !cfResponse)
    return jsonResp({ ok: false, message: "参数缺失" }, 400);

  const stateKey = `ts:${linkToken}`;
  const state = await safeGetJSON(env, stateKey, null);
  if (!state || state.userId === undefined) {
    return jsonResp(
      { ok: false, message: "验证链接已过期，请返回聊天窗口重新发送消息" },
      410,
    );
  }
  ...
  try {
    ...
    return jsonResp(
      { ok: false, message: "验证服务暂时不可用，请稍后重试" },
      502,
    );
  }
  if (!svResult || !svResult.success) {
    Logger.warn("turnstile_verify_failed", {
      userId,
      errors: svResult && svResult["error-codes"],
    });
...
const SPAM_PATTERNS = [
  ...
  { weight: 25, name: "外部链接", re: /(?:https?:\/\/|www\.)\S+/i },
  {
    weight: 40,
    name: "社交联系方式",
    re: /(?:微信|vx|v信|威信|薇信|wx|扣扣|qq)\s*[:：号]?\s*[a-z0-9_-]{5,}/i,
  },
...
function analyzeContentSignals(msg) {
  ...
  const urlCount = entities.filter(
...
async function handleCleanupCommand(threadId, env) {
  ...
  await env.TOPIC_MAP.put(lockKey, "1", {
    expirationTtl: CONFIG.CLEANUP_LOCK_TTL_SECONDS,
  });
        ...
        const results = await Promise.allSettled(
          batch.map(async (name) => {
            ...
            if (probe.status === "redirected" || probe.status === "missing") {
              ...
              Logger.warn("cleanup_probe_missing_thread_id", {
                userId,
                threadId: topicThreadId,
              });
            }
...
async function createTopic(from, key, env, userId) {
  ...
  if (!env.SUPERGROUP_ID.toString().startsWith("-100"))
    throw new Error("SUPERGROUP_ID必须以-100开头");
  const res = await tgCall(env, "createForumTopic", {
    chat_id: env.SUPERGROUP_ID,
    name: title,
  });
...
async function handleMediaGroup(
  ...
  await env.TOPIC_MAP.put(key, JSON.stringify(rec), {
    expirationTtl: CONFIG.MEDIA_GROUP_EXPIRE_SECONDS,
  });
